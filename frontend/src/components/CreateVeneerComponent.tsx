...
const CreateVeneerComponent = ({ back }: { back: () => void }) => {
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setData((prevData: any) => ({
      ...prevData,
      value: `${data.specie.split(" ").join("-").toLowerCase()}_${data.cut.split(" ").join("-").toLowerCase()}_${data.match.split(" ").join("-").toLowerCase()}_${name === 'grade' ? value.toLowerCase() : data.grade.toLowerCase()}`,
      [name]: value
    }));
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!data.specie || !file || !data.cut || !data.match || !data.grade) {
    ...
    try {
      setIsLoading(true);
      await CreateVeneer(formData, dispatch);
      setData({
        specie: "",
        cut: "",
        match: "",
      ...
      console.error("CreateCore error:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className='w-full h-full flex flex-col !p-2'>
      <h1 className='text-gray-800 text-2xl font-semibold relative flex items-center '>
        <button className='hover:text-red-600 cursor-pointer ' onClick={back}>
      ...
      >
        <div className='flex-1'>
          <label htmlFor="specie" className='text-gray-600 font-semibold'>Veneer Specie</label>
          <select
            id='specie'
            name='specie'
            value={data.specie}
          ...
          >
            <option value="" disabled>Select Veneer Specie</option>
        ...
        <div className='flex-1'>
          <label htmlFor="cut" className='text-gray-600 font-semibold'>Cut</label>
          <select
            id='cut'
            name='cut'
            value={data.cut}
        ...
        <div className='flex-1'>
          <label htmlFor="match" className='text-gray-600 font-semibold'>Match</label>
          <select
            id='match'
            name='match'
            value={data.match}
        ...
        <div className='flex-1'>
          <label htmlFor="grade" className='text-gray-600 font-semibold'>Grade</label>
          <select
            id='grade'
            name='grade'
            value={data.grade}
        ...
        <div className='flex-1'>
          <label htmlFor="file" className='text-gray-600 font-semibold'>Texture File</label>
          <input
            id='file'
            name='file'
            type="file"
        ...
        >
          {isLoading ? 'Creating...' : 'Create Veneer'}
        </button>
      </form>
      <div className='w-full flex flex-col gap-3 !px-2 !mt-8'>
        ...
        <div className='w-full h-[400px] bg-gray-200 rounded-lg flex items-center justify-center'>
          {preview ? (
            <img src={preview} alt='Core Texture' className='w-full h-full object-cover rounded-lg' />
          ) : (
...
export default CreateVeneerComponent;
